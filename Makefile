...
test: ## Run PHPUnit for the unit tests
	docker-compose run --rm --no-deps worm-php /bin/sh -c "./vendor/bin/phpunit $(if $(TEST),$(TEST),)"

phpstan: ## Run PHPStan to perform static analysis
	docker-compose run --rm --no-deps worm-php /bin/sh -c "./vendor/bin/phpstan analyse --level 8 src tests"
